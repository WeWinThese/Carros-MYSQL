...
public function getAll() {
    try {
        $query = "SELECT * FROM JuniorCarros";
...
public function getById($id) {
    try {
        $query = "SELECT * FROM JuniorCarros WHERE id = :id";
        $stmt = $this->conn->prepare($query);
...
public function create() {
    try {
        $query = "INSERT INTO JuniorCarros (marca, detalhes, foto) VALUES (:marca, :detalhes, :foto)";
        $stmt = $this->conn->prepare($query);
...
public function update() {
    try {
        ...
        $query = "UPDATE JuniorCarros SET marca = :marca, detalhes = :detalhes, foto = :foto WHERE id = :id";
        $stmt = $this->conn->prepare($query);
...
public function delete($id) {
    try {
        $query = "DELETE FROM JuniorCarros WHERE id = :id";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(':id', $id, \PDO::PARAM_INT);
